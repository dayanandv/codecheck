...
for item in config['test_suite']:
    ...
    for test_case in item['test_cases']:
        ...
        input_file = str(test_case['input_file'])
        output_file = str(test_case['output_file'])
    ...
    print(f"{item['code_file']} : {cs.format('BOLD', test_case_pass_count[str(item)]/test_case_count[str(item)]*100)}% test cases passed.")
